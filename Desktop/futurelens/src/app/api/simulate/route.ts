...
import { checkInputSafety, validateSimulationOutput } from "@/lib/safety";
import type {
  CareerProjection,
  FinancialProjection,
  WellnessProjection,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ─── Safety: Check input for harmful content ───
    const inputText = Object.values(profile)
      .filter((v) => typeof v === "string")
      .join(" ");
    const inputSafety = checkInputSafety(inputText);
    if (inputSafety.blocked) {
      return NextResponse.json(
        { error: inputSafety.message, safetyBlock: true },
        { status: 400 }
      );
    }
...
${JSON.stringify(lifestyleResult, null, 2)}`;
    ...
    const outputSafety = validateSimulationOutput(result);
    const resultWithSafety = {
      ...result,
      disclaimers: outputSafety.disclaimers,
      safetyIssues: outputSafety.issues.length > 0 ? outputSafety.issues : undefined,
    };

    return NextResponse.json(resultWithSafety);
  } catch (error) {
