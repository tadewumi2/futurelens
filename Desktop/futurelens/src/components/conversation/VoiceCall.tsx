...
export default function VoiceCall() {
  ...
  const { saveTranscript: persistTranscript } = useSession();
  ...
  useEffect(() => {
    ...
    socket.on("voice:transcript", (data: { text: string; role: string }) => {
      ...
      // Crisis detection on user speech
      if (role === "USER") {
        const crisis = detectCrisis(data.text);
        if (crisis.crisisDetected && crisis.response) {
          setCrisisAlert(crisis.response);
        }
      }
